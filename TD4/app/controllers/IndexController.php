...
class IndexController extends ControllerBase{
    ...
    {
        $semantic = $this->jquery->semantic();
        $bt = $semantic->htmlButton("btText","Test");
        $bt->addIcon("user");
        $bt->setProperty("data-ajax","importante !");
        $bt->getOnClick("index/response","#divreponse",["attr"=>"data-ajax","ajaxTransition"=>"random"]);
        $bt->compile($this->jquery);

        $menu = $semantic->htmlLabeledIconMenu("menu2",array(["star","star"],["user","user"],["video","video"]));
        $menu->onClick("$('#divResponse').html($(this).attr('id'));");
        $menu->getOnClick("index/response","#divreponse");
        $this->jquery->compile($this->view);
    }
    ...
    {
        $semantic = $this->jquery->semantic();
        $info = $semantic->htmlMessage("mess","Response Ok");
